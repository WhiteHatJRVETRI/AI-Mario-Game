...
function  startgame(){
gamestatus="start"
document.getElementById("status").innerHTML="Wait Game Is Loading"
}
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press The Play Game ↑↑ Above Start and Play ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  ...
  if(gameConfig.status==="gameove  r" && keyDown(control.revive)) {
